...
class StudentClass:
    def __init__(self, root):
        ...
        # state er jonno lbl_state = Label(self.root, text="State", font=(
        # "goudy old style", 15, "bold"), bg="white").place(x=10, y=220)
        ...
        txt_city = Entry(self.root, textvariable=self.var_city, font=(
            "goudy old style", 15, "bold"), bg="lightyellow").place(x=380, y=220, width=100)

        # pin er jonno
        lbl_pin = Label(self.root, text="Pin", font=(
            "goudy old style", 15, "bold"), bg="white").place(x=500, y=220)
        ...
        self.txt_gender = ttk.Combobox(self.root, textvariable=self.var_gender, values=("Select", "Male", "Female", "Others"), font=(
        ...
        lbl_contact = Label(self.root, text="Contact", font=(
            "goudy old style", 15, "bold"), bg="white").place(x=360, y=100)
        lbl_admission = Label(self.root, text="Admission", font=(
            "goudy old style", 15, "bold"), bg="white").place(x=360, y=140)
        lbl_course = Label(self.root, text="Course", font=(
            "goudy old style", 15, "bold"), bg="white").place(x=360, y=180)

        # ======Entry Fields 2=======
        #self.course_list = ["Empty"]
        self.course_list = []
        # function call to update this list
        ...
        txt_contact = Entry(self.root, textvariable=self.var_contact, font=(
            "goudy old style", 15, "bold"), bg="lightyellow").place(x=480, y=100, width=200)
        txt_admission = Entry(self.root, textvariable=self.var_a_date, font=(
            "goudy old style", 15, "bold"), bg="lightyellow").place(x=480, y=140, width=200)
       ...
       # ==========txt Address=========
        ...
        # ====show er ta call====
        self.show()
        # ai function bolecilam function call update otar nice dibo self.fetch_course()
...
# ==========================================================================
        ...
        try:
            if self.var_roll.get() == "":
                messagebox.showerror(
                    "Error", "Roll No Name should be required", parent=self.root)
            else:
                cur.execute(
                ...
                if row == None:
                    messagebox.showerror(
                        "Error", "Please select student from the lish first ", parent=self.root)
                else:
                    ...
                    if op == True:
                        ...
                        con.commit()
                        messagebox.showinfo(
                            "Delete", "Student deleted Sucessfully", parent=self.root)
                        self.clear()
    ...
    def add(self):
        ...
        try:
            if self.var_roll.get() == "":
                messagebox.showerror(
                    "Error", "Roll Number should be required", parent=self.root)
            else:
                cur.execute(
                    "select * from student where roll=?", (self.var_roll.get(),))
                row = cur.fetchone()
                if row != None:
                    messagebox.showerror(
                        "Error", "Roll No already present", parent=self.root)
                else:
        ...
        try:
            if self.var_roll.get() == "":
                ...
                if row == None:
                    ...
                    con.commit()
                    messagebox.showinfo(
                        "Success", "Student Update Successfully", parent=self.root)
    ...
    def show(self):
        ...
        try:
            ...
            messagebox.showerror("Error", f"Error due to {str(ex)}")
# course name fetch korar jonno course theke
